...
// Mock data for location and joined year to match the screenshot design
const LOCATIONS = [
  "Pandesara, Surat",
  "Athwa, Surat",
  "Varachha, Surat",
      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 transition-all duration-240">
          {filtered.map((member, i) => (
            <article
              key={member.name}
              className="bg-white rounded-[20px] overflow-hidden shadow-[0_8px_24px_-12px_rgba(14,58,92,0.1)] flex flex-col h-full border border-line/50"
            >
              ...
              <div className="relative h-[260px] w-full shrink-0">
                ...
                {/* Name and Convention */}
                <div className="absolute bottom-[16px] left-[20px] right-[20px] flex flex-col">
                  <h3 className="text-white text-[18px] font-[700] tracking-tight m-0 leading-tight">
                    {member.name}
                  </h3>
                  <p className="text-white/85 text-[13px] font-[500] italic mt-[2px] m-0">
                    {member.convention}
                  </p>
                </div>
              </div>
              ...
              <div className="p-[20px] flex flex-col flex-1">
                {/* Meta info */}
                <div className="flex items-center gap-[10px] text-ink-3 text-[11px] font-[600] mb-[12px] tracking-wide">
                  <div className="flex items-center gap-[4px]">
                    <MapPin size={11} className="text-brand" />
                    {LOCATIONS[i % LOCATIONS.length]}
                  </div>
                  <div className="w-[3px] h-[3px] rounded-full bg-line" />
                  <div className="flex items-center gap-[4px]">
                    <Calendar size={11} className="text-brand" />
                    {JOINED_YEARS[i % JOINED_YEARS.length]}
                  </div>
                </div>

                {/* One-liner */}
                <p className="text-[13px] text-ink-3 leading-[1.6] m-0 text-pretty">
                  {member.oneliner}
                </p>
                ...
                <div className="pt-[16px] border-t border-line flex items-center gap-[8px]">
                  <a
                    href="#"
                    className="w-[28px] h-[28px] rounded-full bg-paper flex items-center justify-center text-ink-3 hover:text-brand hover:bg-brand-soft transition-colors cursor-pointer"
                  >
