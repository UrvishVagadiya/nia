...
const MemberCard = ({ member }: { member: Member }) => {
  return (
    <div className="h-full hover:-translate-y-1.5 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]">
      <Card className="bg-white rounded-lg overflow-hidden shadow-[0_8px_24px_-12px_rgba(14,58,92,0.1)] flex flex-col h-full border border-line/50 !p-0 gap-0 ring-0 transition-shadow duration-300 hover:shadow-[0_16px_36px_-12px_rgba(14,58,92,0.18)]">
        {/* Image Section */}
        <div className="relative h-72 sm:h-80 w-full shrink-0 bg-paper-3">
          <Image
            src={
              (typeof member.photo === "object" ? member.photo?.url : member.photo) ||
              (member as { photoURL?: string }).photoURL ||
